...
# Path for the target Auth component
my $auth_file = 'src/components/Auth.tsx';

# The content is a merge of the "Grandfather" logic and the "Addis Neon" UI
my $grandfather_content = <<'GRANDFATHER_EOF';
...
open(my $fh, '>', $auth_file) or die "Could not open $auth_file: $!";
print $fh $grandfather_content;
close($fh);
print "✅ Grandfather Auth logic applied to $auth_file\n";
